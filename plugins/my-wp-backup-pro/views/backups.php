...
<div class="page-backups wrap" id="my-wp-backup">
    ...
    <?php if ( isset( $_GET['uniqid'] ) && isset( $_GET['id'] ) && isset( $_GET['action'] ) ) : ?>
	    ...
	    <?php elseif ( 'debug' === $action ) : ?>

		    <?php $file = \MyWPBackup\Admin\Backup::get_instance()->get( $id, $uniqid )->get_debug_log(); ?>
		    <?php
		    try {
			    $file_object = $file->openFile();
		    ?>
				<pre class="terminal"><code><?php while ( $line = $file_object->fgets() ) {echo esc_html( $line ); } ?></code>
				</pre>
		    <?php
		    } catch ( \Exception $e ) {
			    esc_html_e( 'Unable to open debug file.', 'my-wp-backup' );
		    }
		    ?>
