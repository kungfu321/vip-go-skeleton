...
class MTS_RMU {
	...
	private function __construct( $config = array() ) {
		$config_defaults = array(
			...
			'link_label_install'      => __( 'Try it for FREE!', 'cyprus' ),
			'link_label_activate'     => __( 'Click here to activate it.', 'cyprus' ),

			// Add tab in MTS Theme Options.
			'add_theme_options_tab'   => true,
			'theme_options_notice'    => '<span style="display: block; line-height: 1.8; margin-bottom: 20px;">' . sprintf( __( '%1$s is a revolutionary SEO product that combines the features of many SEO tools. Its features give you the power of an entire SEO team with just a few clicks.', 'cyprus' ), '<a href="https://mythemeshop.com/plugins/wordpress-seo-plugin/?utm_source=Theme+Options+Panel&utm_medium=Link+CPC&utm_content=Rank+Math+SEO+LP&utm_campaign=UserBackend" target="_blank" style="color: #0073aa;">Rank Math SEO</a>' ) . ' @CTA' . '</span>',
			...
			'metabox_notice_install'  => sprintf( __( 'The new %1$s plugin will help you rank better in the search results.', 'cyprus' ), '<a href="https://mythemeshop.com/plugins/wordpress-seo-plugin/?utm_source=SEO+Meta+Box&utm_medium=Link+CPC&utm_content=Rank+Math+SEO+LP&utm_campaign=UserBackend" target="_blank">Rank Math SEO</a>' ) . ' @CTA',

			/* Translators: %s is CTA, e.g. "Try it now!" */
			'metabox_notice_activate' => sprintf( __( 'The %1$s plugin is installed but not activated.', 'cyprus' ), '<a href="https://mythemeshop.com/plugins/wordpress-seo-plugin/?utm_source=SEO+Meta+Box&utm_medium=Link+CPC&utm_content=Rank+Math+SEO+LP&utm_campaign=UserBackend" target="_blank">Rank Math SEO</a>' ) . ' @CTA',
	...
	public function add_dashboard_widget() {
		...
		wp_add_dashboard_widget( 'rmu_dashboard_widget', __( 'Rank Math SEO', 'cyprus' ), array( $this, 'dashboard_widget_output' ) );
	}
	public function enqueue_install_scripts() {
	...
	public function dashboard_widget_output( $post, $callback_args ) {
		...
		?>
			<div class="rmu-dashboard-panel">
				<a class="rmu-dashboard-panel-close" id="rmu-dashboard-dismiss" href="http://cyprus.local/wp-admin/?rmu-dashboard=0""><?php _e( 'Dismiss', 'cyprus' ); ?></a>
	...
	public function get_install_link( $class = '', $label = '' ) {
		if ( ! $label ) {
			$label = '<strong style="color: #0073aa;">' . $this->get_setting( 'link_label_install' ) . '</strong>';
		}
		...
		return '<a href="' . $install_link . '" class="thickbox ' . esc_attr( $class ) . '" title="' . esc_attr__( 'Rank Math SEO', 'cyprus' ) . '">' . $label . '</a>';
	}

	public function get_activate_link( $class = '', $label = '' ) {
		if ( ! $label ) {
			$label = '<strong style="color: #0073aa;">' . $this->get_setting( 'link_label_activate' ) . '</strong>';
		}
	...
	public function add_theme_options_seo_tab_content( $sections ) {
		$didyouknow     = '';
		$active_plugins = get_option( 'active_plugins' );
		if ( in_array( 'wordpress-seo/wp-seo.php', $active_plugins, true ) ) {
		...
		$sections['seo'] = array(
			array(
				...
				'desc'  => strtr( $this->get_setting( 'theme_options_notice' ), array( '@CTA' => $this->get_install_or_activate_link() ) ) . $didyouknow . $this->get_install_or_activate_link( 'button button-primary', __( 'Install Now', 'cyprus' ), __( 'Activate It', 'cyprus' ) ) . $this->get_notice_css(),
			)
		);
		return $sections;
	}

	public function add_theme_options_seo_tab( $menus ) {
		if ( isset( $menus['ad-management'] ) ) {
			$menus['seo'] = array(
				'icon'  => 'fa fa-bar-chart-o',
				'title' => esc_html__( 'SEO', 'cyprus' ),
				'desc'  => esc_html__( 'Fix all your SEO issues with just a few clicks. Comply with all the latest Google’s guidelines.', 'cyprus' ),
			);
		}
		return $menus;
	}
	...
	public function filter_plugins_popup( $result, $action, $args ) {
		if ( ! isset( $args->slug ) || $args->slug != RMU_PLUGIN_SLUG || $action != 'plugin_information' ) {
			return false;
		}
...
<li><p><strong>Google Cyprus Rich Snippets Integrated</strong>
...
<li>1 Click Import From All In One Cyprus Rich Snippets</li></ul>
</li>

<li><strong>Role Manager</strong></li>

<li><strong>Google Cyprus Rich Snippets Integrated</strong>
