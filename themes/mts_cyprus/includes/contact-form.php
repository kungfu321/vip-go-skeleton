	...
	class mtscontact {
		...
		public function ajax_mtscontact() {
			if ($this->validate()) {
				if ($this->send_mail()) {
					...
					$this->errors['sendmail'] = __('An error occurred. Please contact site administrator.', 'cyprus' );
		...
		public function register_scripts() {
			...
			array(
				...
				'alertmsg'   => __('Please check the consent checkbox if you want to proceed.', 'cyprus')
			));
		}
		...
		private function validate() {
			...
			if (!check_ajax_referer( 'mtscontact', 'mtscontact_nonce', false )) {
				$this->errors['nonce'] = __('Please try again.', 'cyprus' );
			}

			// check honeypot // must be empty
			if (!empty($_POST['mtscontact_captcha'])) {
				$this->errors['captcha'] = __('Please try again.', 'cyprus' );
			}

			// name field
			$name = trim(str_replace(array("\n", "\r", "<", ">"), '', strip_tags($_POST['mtscontact_name'])));
			if (empty($name)) {
				$this->errors['name'] = __('Please enter your name.', 'cyprus' );
			}

			// email field
			$useremail = trim($_POST['mtscontact_email']);
			if (!is_email($useremail)) {
				$this->errors['email'] = __('Please enter a valid email address.', 'cyprus' );
			}

			// message field
			$message = strip_tags($_POST['mtscontact_message']);
			if (empty($message)) {
				$this->errors['message'] = __('Please enter a message.', 'cyprus' );
			}
		...
		private function send_mail() {
			...
			$email_subject = __('Contact Form Message from', 'cyprus' ).' '.get_bloginfo('name');
			$email_message = __('Name:', 'cyprus' ).' '.$this->userinput['name']."\n\n".
							 __('Email:', 'cyprus' ).' '.$this->userinput['email']."\n\n".
							 __('Message:', 'cyprus' ).' '.$this->userinput['message'];
		...
		public function get_form() {
			...
			if (!$this->success) {
				...
				<div class="mtscontact_consent">
					...
					<label for="mtscontact_consent">'.__('Please tick to agree to be contacted.', 'cyprus').'</label>
				</div>

				<input type="submit" value="'.__('Send', 'cyprus' ).'" id="mtscontact_submit" />
			</form>';
			}
			$return .= '<div id="mtscontact_success"'.($this->success ? '' : ' style="display: none;"').'>'.__('Your message has been sent.', 'cyprus' ).'</div>';
			return $return;
		}
