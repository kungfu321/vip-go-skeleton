	...
	class mts_related_posts_widget extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				sprintf( __('%sRelated Posts', 'cyprus' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
				array( 'description' => __( 'Display the related posts from current post\'s categories. Will appear on single posts only.', 'cyprus' ) )
			);
		...
		public function form( $instance ) {
			...
			$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : __( 'Related Posts', 'cyprus' );
			...
			<p>
				<label for="<?php echo $this->get_field_id("comment_num"); ?>">
					...
					<?php _e( 'Show number of comments', 'cyprus' ); ?></p>
				</label>
			</p>
		...
		public function widget( $args, $instance ) {
			...
			if(is_singular()){
				...
				echo self::get_cat_posts( $qty, $title_length, $comment_num, $date, $show_thumb6, $box_layout, $border_radius, $show_excerpt, $excerpt_length );
				echo $after_widget;
			}
		}

		public function get_cat_posts( $qty, $title_length, $comment_num, $date, $show_thumb6, $box_layout, $border_radius, $show_excerpt, $excerpt_length ) {
